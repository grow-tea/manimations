...
class Radiany_na_stupne(Slide):
    ...
    def construct(self):
        ...
        for i in range(len(radiany)):
            ...
            self.wait(1)
            self.next_slide()
            vypocet = MathTex(r"\frac{" + radiany_tex[i] + "}{2 \pi} = " + zlomky[i], **gs.TEXT).move_to(k.misto_pro_pocitani)
            ...
            vypocet2 = MathTex(zlomky[i] + r"\cdot 360^{\circ} =" + str(k.get_stupne()) + r"^{\circ}", **gs.TEXT).next_to(vypocet, DOWN, aligned_edge=LEFT)
