...
class Sinus(Slide):
    ...
    def construct(self):
        ...
        self.add(g.kruznice_grupa, g.k.sin_grupa, g.graf_grupa)
        ...
        for uhel in zachytne:
            ...
            self.wait(1)
            self.next_slide()
            self.play(FadeIn(g.bod_sinusoida.update()))
            self.play(Write(g.pomocna_usecka.update()), run_time=1)
            self.add(g.bod_sinusoida.copy())
            self.wait(1)
            self.next_slide()
            self.play(Unwrite(g.bod_sinusoida), Unwrite(g.pomocna_usecka), FadeOut(g.svislice), run_time=1)
            self.wait(1)


        self.next_slide()
        for uhel in zachytne_fast:
            self.play(FadeIn(Dot(g.osy_grafu.c2p(uhel, np.sin(uhel)), **gs.SIN_BOD)))        
        self.wait(1)
        self.next_slide()

        self.play(g.k.velikost_uhlu.animate.set_value(0.1*DEGREES), run_time=3)
        self.add(g.bod_sinusoida.update(), g.pomocna_usecka.update(), g.sinusoida_krivka)
